...
int hysteresis = 10; //гистерезис -- hysteresis

bool hall_sensor1, hall_sensor2; //переменная датчиков холла -- hall effect signal flags
...
void setup() {
  ...
  hall_sensor1 = false; //устанавливает выход сигнала  имитация датчик холла 1 в низкий логический уровень -- sets flag for sensor simulator
  ...
  hall_sensor2 = false; //устанавливает выход сигнала  имитация датчик холла 2 в низкий логический уровень -- sets flag for sensor simulator
  //period = 50;  
}
...
void loop() {
  ...
  if (but_state == 1) //проверка чтения кнопки если лог1 то читаем аналоговый вход, если лог0 не читаем -- checking for button state, 1 - proceed to read pot, 0 - exit
  {
    ...
    if (pot_val > (512+hysteresis)) {freq = map(pot_val, (512+hysteresis), 1023, 1, 250);} // диапазон чтения 1,1023 положение резюка -140, 140 частота в герцах -- remapping raw pot input into frequency in Hz
        else if (pot_val < (512-hysteresis)) {freq = map(pot_val, 1, (512-hysteresis), -250, -1);}
    ...
    {
      delay(period/4);
      hall_sensor1 = !hall_sensor1; //логика работы выходов -- output switching logic
      digitalWrite(hall_1_pin, hall_sensor1);

      delay(period/4);
      hall_sensor2 = !hall_sensor2;
      digitalWrite(hall_2_pin, hall_sensor2);
    } 
    
    else if (freq < min_freq) //почему меньше чем -мин_частота?! -- why less that -freq?! original "else if (freq < (-min_freq))"
    {
      delay(period/4);
      hall_sensor2 = !hall_sensor2;
      digitalWrite(hall_2_pin, hall_sensor2);
